...
class GitFile:
    def __init__(self, filename):
        ...
        if exitcode != 0:
            raise GitException(
                "\"%s\" is not inside a git directory!" % self.filename
            )
        self.remote = None
        self.branch = None
        self.commit = None
    ...
    def git_command(self, command):
        git = subprocess.Popen(
            'git %s' % command, shell=True, stdout=subprocess.PIPE,
    ...
    def get_remote(self):
        if self.remote is None:
            self.remote = re.sub(
                r'\.git$', '',
                self.git_command("config --get remote.origin.url")[0].strip()
            )
        return self.remote

    def get_branch(self):
        if self.branch is None:
            self.branch = self.git_command("rev-parse --abbrev-ref HEAD")[0].strip()
        return self.branch
    ...
    def get_path(self):
        if self.path is None:
            self.path = re.sub(
                r"^/", "",
                self.filename.replace(self.get_root(), "")
            )
        return self.path
    ...
    def get_ref(self):
        if self.ref is None:
            self.ref = (self.get_commit() if self.get_branch() == "HEAD"
                        else self.get_branch())
        return self.ref
    ...
    def open(self, selection=None):
        for host in hosts:
            ...
            if h.matches:
                ...
                return
        raise GitException(self.get_remote() + " is not a supported git host!")
