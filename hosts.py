class Host:
    ...
    def matches(self, hostname):
        return (hostname.lower() in self.host.lower() or
                self.host.lower() in hostname.lower())
...
class GitHub(Host):
    ...
    def format_url(self, selection=None):
        if self.isfile:
            if selection:
                lines = "#" + ("L%d-L%d" % selection if len(selection) == 2
                               else "L%d" % selection)
            else:
                lines = ""
            return "https://{host}/blob/{ref}/{path}{lines}".format(
                host=self.host,
                ref=self.ref,
                path=self.path,
                lines=lines
            ...
            return "https://{host}/tree/{ref}/{path}".format(
                host=self.host,
                ref=self.ref,
                path=self.path
            )
...
class BitBucket(Host):
    ...
    def format_url(self, selection=None):
        if selection:
            lines = "#-" + ("%d:%d" % selection if len(selection) == 2
                            else "%d" % selection)
        else:
            lines = ""
        return "https://{host}/src/{ref}/{path}{lines}".format(
            host=self.host,
            ref=self.ref,
            path=self.path,
            lines=lines
